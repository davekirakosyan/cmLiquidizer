...
public class RopeRoot : MonoBehaviour
{
    ...
    private void AddChildren(Transform parent)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            Transform testParent = parent;
            while (testParent.transform.parent)
            { // This checks if it's null or not.
                testParent = testParent.transform.parent;
            }
            var representative = new GameObject(testParent.name+" "+child.gameObject.name);
